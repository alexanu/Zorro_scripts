...
function run()
{
	...
	// measure the time
	static var Time = 0;
	if(Train && is(FIRSTINITRUN)) Time = timer();
	if(Test && is(INITRUN) && is(TESTNOW)) printf("\nTime for optimizing >>> %.3f sec <<<",(timer()-Time)/1000);
	...
	while(asset(loop("EUR/USD","USD/JPY","GBP/USD"))) 
	{
		...
		if(crossUnder(Signal,-Threshold)) enterLong(); 
		else if(crossOver(Signal,Threshold)) enterShort();
	}
} 